    ...
    def extract_features(tokens, buffer, stack, arcs):
        ...
        if stack:
            ...
            if len(stack) > 1:
                ...
                if FeatureExtractor._check_informative(token1['tag'], True):
                    result.append('STK_1_TAG_' + token1['tag'])
        ...
        if buffer:
            ...
            if len(buffer) > 1:
                ...
                if FeatureExtractor._check_informative(token1['tag'], True):
                    result.append('BUF_1_TAG_' + token1['tag'])
            ...
            if len(buffer) > 2:
                ...
                if FeatureExtractor._check_informative(token2['tag'], True):
                    result.append('BUF_2_TAG_' + token2['tag'])

            if len(buffer) > 3:
                buffer_idx3 = buffer[3]
                token3 = tokens[buffer_idx3]

                if FeatureExtractor._check_informative(token3['tag'], True):
                    result.append('BUF_3_TAG_' + token3['tag'])
        return result
